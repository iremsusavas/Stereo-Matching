...
#pragma omp parallel for
  ...
  void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter) {
	...
	int row = views_[cpv].rows;
    	int col = views_[cpv].cols;
  ...
  // if iter is odd then the function check the right and lower neighbours
    	std::vector<std::pair<int,int>> disparities;
    	if(iter%2==0)
    	{
    		disparities.push_back(std::make_pair(-60,0));
    		disparities.push_back(std::make_pair(0,-60));
    	...
    	float& disparity = disps_[cpv].at<float>(y, x);
    	float& cost_old = costs_[cpv].at<float>(y, x);
   
    	for(auto it = disparities.begin(); it < disparities.end(); ++it)
			{
				std::pair<int, int> disps = *it;
			
				int ny = y + disps.first;
				int nx = x + disps.second;
			//check if x and y values are valid
				if(!inside(nx, ny, 0, 0, col, row))
					continue;
				float& disparity_neigh = disps_[cpv].at<float>(ny, nx);
				float new_cost = precomputed_disp_match_cost(disparity_neigh, x, y, cpv);
//check if the new cost is smaller than the old cost if so assign the new values for the disparity and old cost
				if(new_cost < cost_old)
				{	disparity = disparity_neigh;
					cost_old = new_cost;
				}
  	}
}
 //INPUT:
  ...
  void PatchMatch::view_propagation(int x, int y, int cpv) {
	int row = views_[cpv].rows;
    	...
    	float& disparity = disps_[cpv].at<float>(y, x);
    	float& cost_old = costs_[cpv].at<float>(y, x);
    	float new_disparity = disparity+ 10.0f; //adding a factor of delta_z selected as 10.0f
    	float new_cost = precomputed_disp_match_cost(new_disparity, x, y, cpv);
    	if(!inside(x, y, 0, 0, col, row))
		return;
		
		
	//if new cost is smaller than old cost, assing the new values to disparity and old cost	
	if(new_cost < cost_old)
		{
  ...
  void PatchMatch::disp_perturbation(int x, int y, int cpv, float max_delta_z, float end_dz) {
	...
	float& old_disparity = disps_[cpv](y, x);
	float& old_cost = costs_[cpv](y, x);
		
	while(max_dz >= end_dz) //cycle between max_dz and end_dz
	{
		// Searching a random disparity starting from the actual one
		std::random_device rd; //uniformly distributed integer random number
		std::mt19937 gen(rd()); //pseuodo random generator
			
		std::uniform_real_distribution<> rand_z(-max_dz, +max_dz);
		
		float delta_z = rand_z(gen); //assing a random number to delta_z
		float new_disparity = old_disparity+ delta_z; //add delta_z to the old disparity
